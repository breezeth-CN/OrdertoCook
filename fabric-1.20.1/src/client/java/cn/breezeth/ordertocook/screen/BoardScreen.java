...
public class BoardScreen extends HandledScreen<BoardScreenHandler> {
    private static final Identifier BG_TEXTURE = new Identifier("ordertocook", "textures/gui/board_ui.png");
    private static final Identifier SLOT_BG = new Identifier("ordertocook", "textures/gui/board_item_bg.png");
    private static final Identifier SLOT_HOVER_NORMAL = new Identifier("ordertocook", "textures/gui/board_highlight.png");
    private static final Identifier SLOT_HOVER_DELETE = new Identifier("ordertocook", "textures/gui/board_delete.png");
    ...
    /** 与 fabric-1.21.1 BoardScreen 一致：JEI {@code getGuiExtraAreas} 使用与绘制相同的 gui 原点。 */
    ...
    protected void init() {
        ...
        cn.breezeth.ordertocook.compat.JeiCompat.hideOverlay();
    ...
    public int oc$getButtonX() {
        return ((this.width - this.backgroundWidth) / 2) + this.backgroundWidth + 4;
    }
    public int oc$getButtonY() {
        return ((this.height - this.backgroundHeight) / 2) + 18;
    }
    ...
    protected void handledScreenTick() {
        ...
        cn.breezeth.ordertocook.compat.JeiCompat.ensureGuiHandlersRegistered();
        // 检查是否需要更新按钮位置（例如窗口大小改变时）
        if (editButton != null) {
            int expectedX = oc$getGuiLeft() + this.backgroundWidth + 4;
            int expectedY = oc$getGuiTop() + 18;
            if (editButton.getX() != expectedX || editButton.getY() != expectedY) {
                updateButtonPosition();
            }
        }
        if (sortButton != null) {
