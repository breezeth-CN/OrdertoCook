...
public class FoodPlateBlock extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        ...
        return (BlockEntityTicker<T>) (world1, pos1, state1, blockEntity) -> {
            if (blockEntity instanceof FoodPlateBlockEntity foodPlateBe) {
                FoodPlateBlockEntity.tick(world1, pos1, state1, foodPlateBe);
            }
        };
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (state.contains(STAGE) && state.get(STAGE) == 4) {
            world.playSound(null, pos, ModSounds.PLATE_PLACE, net.minecraft.sound.SoundCategory.BLOCKS, 0.75f, 0.95f + world.random.nextFloat() * 0.1f);
        } else {
            world.playSound(null, pos, ModSounds.FOOD_PLATE_PLACE, net.minecraft.sound.SoundCategory.BLOCKS, 1.0f, 1.0f);
        }
