...
public final class RestaurantRegistry {
    ...
    private static final Identifier GOLDEN_RESTAURANT_ADVANCEMENT = new Identifier(ModConstants.MOD_ID, "ordertocook/golden_restaurant");
    ...
    /**
     * 与 fabric-1.21.1 一致：优先返回在线缓存，否则从持久化 {@link MachineRankingState} 构造无坐标统计。
     */
    ...
    /** 与 fabric-1.21.1 {@code RestaurantRegistry#applyProfitBonusById} 行为一致：只加利润，不增加接单计数。 */
    public static void applyProfitBonusById(ServerWorld world, int id, int coin, boolean delivery, boolean longDistance, int deliveryDist, boolean walkIn) {
        if (coin <= 0) {
            return;
        }
        MachineRankingState state = MachineRankingState.get(world);
