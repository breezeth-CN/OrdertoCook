...
public final class NpcNames {
    ...
    public static String random(net.minecraft.util.math.random.Random random) {
        ensureLoaded();
        List<String> list = names;
        if (list.isEmpty()) return net.minecraft.text.Text.translatable("keyword.ordertocook.customer").getString();
    ...
    private static List<String> parseNames(ResourceManager resourceManager, String path) {
        ...
        try {
            java.util.Optional<Resource> resource = resourceManager.getResource(new Identifier(ModConstants.MOD_ID, path));
            if (resource.isEmpty()) {
                return List.of();
            }
