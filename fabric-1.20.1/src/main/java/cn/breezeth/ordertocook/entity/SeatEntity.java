...
public class SeatEntity extends Entity {
    ...
    protected void initDataTracker() {
        this.dataTracker.startTracking(PARENT_ENTITY_ID, -1);
        this.dataTracker.startTracking(TRACK_OFF_X, 0.0f);
        this.dataTracker.startTracking(TRACK_OFF_Y, 0.0f);
        this.dataTracker.startTracking(TRACK_OFF_Z, 0.0f);
    }
    ...
    public MotorcycleEntity resolveParent() {
        ...
        if (this.parentId != null) {
            Box search = this.getBoundingBox().expand(8.0);
            for (Entity entity : this.getWorld().getEntitiesByClass(MotorcycleEntity.class, search, candidate -> candidate.getUuid().equals(this.parentId))) {
    ...
    /**
     * 与 fabric-1.21.1 SeatEntity 一致：由引擎每 tick 用 PositionUpdater 同步，客户端插值更稳，减少乘客与座位脱节。
     */
    @Override
