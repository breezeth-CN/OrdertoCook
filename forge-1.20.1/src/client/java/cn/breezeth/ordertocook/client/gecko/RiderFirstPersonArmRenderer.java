...
public final class RiderFirstPersonArmRenderer extends GeoObjectRenderer<RiderAnimatable> {
    ...
    public void renderArm(PoseStack matrices, RiderAnimatable animatable, MultiBufferSource vertexConsumers, float tickDelta, boolean leftHand) {
        ...
        RenderType layer = RenderType.entityCutout(animatable.getPlayer().getSkinTextureLocation());
    ...
    @Override
    public void renderRecursively(PoseStack poseStack, RiderAnimatable animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        Minecraft client = Minecraft.getInstance();
