...
public class CustomerEntity extends Zombie implements GeoEntity {
    private static final EntityDataAccessor<Integer> TEXTURE_VARIANT = SynchedEntityData.defineId(CustomerEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<String> SKIN_ACCOUNT = SynchedEntityData.defineId(CustomerEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<String> SKIN_UUID = SynchedEntityData.defineId(CustomerEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Boolean> EASTER_EGG = SynchedEntityData.defineId(CustomerEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> ANIMATION_VARIANT = SynchedEntityData.defineId(CustomerEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<String> CUSTOMER_ID = SynchedEntityData.defineId(CustomerEntity.class, EntityDataSerializers.STRING);
    private static final EntityDataAccessor<Integer> ACTION_STATE = SynchedEntityData.defineId(CustomerEntity.class, EntityDataSerializers.INT);
    ...
    public CustomerEntity(EntityType<? extends Zombie> entityType, Level world) {
        super(entityType, world);
        this.setCanPickUpLoot(false);
    }

    public static AttributeSupplier.Builder createCustomerAttributes() {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(SKIN_ACCOUNT, "");
        this.entityData.define(SKIN_UUID, "");
        this.entityData.define(EASTER_EGG, false);
        this.entityData.define(ANIMATION_VARIANT, 1);
        this.entityData.define(CUSTOMER_ID, "");
        this.entityData.define(ACTION_STATE, ACTION_NONE);
    }
    ...
    public void setTextureVariant(int variant) {
        this.entityData.set(TEXTURE_VARIANT, Math.max(1, variant));
    }
    ...
    public void setSkinAccount(String account) {
        this.entityData.set(SKIN_ACCOUNT, account == null ? "" : account);
    }

    public String getSkinAccount() {
        return this.entityData.get(SKIN_ACCOUNT);
    }

    public void setSkinUuid(String skinUuid) {
        this.entityData.set(SKIN_UUID, skinUuid == null ? "" : skinUuid);
    }

    public String getSkinUuid() {
        return this.entityData.get(SKIN_UUID);
    }
    ...
    public void setEasterEgg(boolean easterEgg) {
        this.entityData.set(EASTER_EGG, easterEgg);
    }

    public boolean isEasterEggCustomer() {
        return this.entityData.get(EASTER_EGG);
    }

    public void setAnimationVariant(int variant) {
        this.entityData.set(ANIMATION_VARIANT, Math.max(1, Math.min(4, variant)));
    }

    public int getAnimationVariant() {
        return this.entityData.get(ANIMATION_VARIANT);
    }

    public void setCustomerId(String customerId) {
        this.entityData.set(CUSTOMER_ID, customerId == null ? "" : customerId);
    }

    public String getCustomerId() {
        return this.entityData.get(CUSTOMER_ID);
    }
    ...
    public void assignRandomAnimationVariant(RandomSource random) {
    ...
    private int rollWeightedAnimationVariant(RandomSource random) {
    ...
    private void tickTeleportEffects(int actionState, int elapsedTicks) {
        ...
        if (this.currentActionSignalTriggered || elapsedTicks < triggerTick || !(this.level() instanceof ServerLevel serverWorld)) {
            return;
        }
        this.currentActionSignalTriggered = true;
        serverWorld.playSound(null, this.blockPosition(), SoundEvents.ENDERMAN_TELEPORT, SoundSource.NEUTRAL, 1.0f, 1.0f);
        serverWorld.sendParticles(ParticleTypes.PORTAL, this.getX(), this.getY() + 0.8, this.getZ(), 40, 0.35, 0.5, 0.35, 0.2);
    }
    ...
    private void tickEatEffects(int actionState, int elapsedTicks) {
        ...
        if (elapsedTicks >= 45 && elapsedTicks < 95 && elapsedTicks % 7 == 0) {
            serverWorld.playSound(null, this.blockPosition(), SoundEvents.GENERIC_EAT, SoundSource.NEUTRAL, 0.7f, 0.9f + this.random.nextFloat() * 0.2f);
        }
        ...
        if (!this.currentActionSignalTriggered && elapsedTicks >= 97) {
            this.currentActionSignalTriggered = true;
            serverWorld.playSound(null, this.blockPosition(), SoundEvents.CREEPER_PRIMED, SoundSource.NEUTRAL, 0.9f, 1.0f);
        }
        if (!this.currentActionFinishTriggered && elapsedTicks >= 118) {
            this.currentActionFinishTriggered = true;
            serverWorld.playSound(null, this.blockPosition(), SoundEvents.GENERIC_EXPLODE, SoundSource.NEUTRAL, 1.0f, 1.0f);
            serverWorld.sendParticles(ParticleTypes.EXPLOSION_EMITTER, this.getX(), this.getY() + 0.8, this.getZ(), 1, 0.0, 0.0, 0.0, 0.0);
            serverWorld.sendParticles(ParticleTypes.CLOUD, this.getX(), this.getY() + 0.8, this.getZ(), 18, 0.3, 0.4, 0.3, 0.05);
        }
    }

    private void grantPendingBoomBonus(ServerLevel world) {
        if (!this.pendingBoomBonus || this.pendingBoomBonusCoin <= 0 || world.getServer() == null) {
            clearPendingBoomBonus();
            return;
        }
        try {
            Player player = this.pendingBoomPlayerUuid.isBlank() ? null : world.getServer().getPlayerList().getPlayer(java.util.UUID.fromString(this.pendingBoomPlayerUuid));
            if (player != null) {
            ...
            String restaurantName = this.pendingBoomRestaurantName.isBlank()
                    ? Component.translatable("keyword.ordertocook.unknown").getString()
            ...
            world.getServer().getPlayerList().broadcastSystemMessage(Component.translatable(
    ...
    public void readAdditionalSaveData(CompoundTag nbt) {
        ...
        if (nbt.contains("ActionState")) {
            this.entityData.set(ACTION_STATE, nbt.getInt("ActionState"));
        }
