...
public final class RiderFirstPersonArmRenderer extends GeoObjectRenderer<RiderAnimatable> {
    ...
    public void renderArm(MatrixStack matrices, RiderAnimatable animatable, VertexConsumerProvider vertexConsumers, float tickDelta, boolean leftHand) {
        ...
        RenderLayer layer = RenderLayer.getEntityCutout(animatable.getPlayer().getSkinTextures().texture());
        VertexConsumer consumer = vertexConsumers.getBuffer(layer);
        int packedLight = WorldRenderer.getLightmapCoordinates(animatable.getPlayer().getWorld(), animatable.getPlayer().getBlockPos());
        this.render(matrices, animatable, vertexConsumers, layer, consumer, packedLight, tickDelta);
        matrices.pop();
    }

    @Override
    public void renderRecursively(MatrixStack poseStack, RiderAnimatable animatable, GeoBone bone, RenderLayer renderType, VertexConsumerProvider bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int renderColor) {
        MinecraftClient client = MinecraftClient.getInstance();
