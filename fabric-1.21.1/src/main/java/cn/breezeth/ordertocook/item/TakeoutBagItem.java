...
public class TakeoutBagItem extends Item {
    ...
    public void appendTooltip(ItemStack stack, net.minecraft.item.Item.TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        if (expiryTick >= 0) {
            if (nowTick >= 0) {
                ...
                if (remainingTicks > 0) {
                    ...
                    tooltip.add(Text.translatable("tooltip.ordertocook.time_left", String.format("%02d:%02d", minutes, seconds))
        ...
        if (nbt.contains("delivery_pos")) {
            ...
            if (pos.contains("x") && pos.contains("z")) {
                tooltip.add(Text.translatable("tooltip.ordertocook.delivery_to", pos.getInt("x"), pos.getInt("z")).formatted(Formatting.BLUE));
            }
        }
    ...
    public static boolean trySubmitDineInNearby(ServerWorld world, PlayerEntity player, ItemStack stack, NbtCompound nbt, SoundEvent submitSound) {
        ...
        if (nearby.isEmpty()) {
            ...
            if (customerName != null && !customerName.isBlank() && !tagged.isEmpty()) {
                ...
                if (byName.size() == 1) {
                    ...
                    if (submitSound != null) {
                        world.playSound(null, npc.getBlockPos(), submitSound, SoundCategory.NEUTRAL, 1.0f, 1.0f);
                    }
    ...
    public static ActionResult trySubmitDineInFromBlockUse(ServerWorld world, PlayerEntity player, ItemStack stack, BlockPos intendedPlatePos, SoundEvent submitSound) {
        NbtCompound nbt = DataCompat.copy(stack);
    ...
    public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
        ...
        if (ConfigManager.isDevModeEnabled()) {
             user.sendMessage(Text.literal("[OTC Dev] useOnEntity -> PASS (Delegate to use)").formatted(Formatting.AQUA), false);
        }
    ...
    public static ActionResult trySubmitDineInFromEntityUse(ServerWorld world, PlayerEntity player, ItemStack stack, Entity interactedEntity, SoundEvent submitSound) {
        NbtCompound nbt = DataCompat.copy(stack);
        if (nbt == null) return ActionResult.PASS;
        ...
        if (customerName != null && !customerName.isBlank()
            ...
            if (ConfigManager.isDevModeEnabled()) {
                player.sendMessage(Text.literal("[OTC Dev] DineIn fallback matched by customerName (entityUse) -> completeDelivery").formatted(Formatting.GRAY), false);
            }
            return submitDineInWithPlacedPlate(world, player, stack, nbt, targetNpc, submitSound)
                    ? ActionResult.SUCCESS
                    : ActionResult.PASS;
    ...
    private static String resolveRestaurantName(ServerWorld world, NbtCompound nbt, int machineId) {
        ...
        if (nbt.contains(ModConstants.NBT_MACHINE_POS) && nbt.contains(ModConstants.NBT_MACHINE_DIM)) {
            try {
                net.minecraft.util.Identifier dimensionId = net.minecraft.util.Identifier.of(nbt.getString(ModConstants.NBT_MACHINE_DIM));
                ...
                if (machineWorld != null) {
                    BlockPos machinePos = BlockPos.fromLong(nbt.getLong(ModConstants.NBT_MACHINE_POS));
    ...
    private static boolean submitDineInWithPlacedPlate(ServerWorld world, PlayerEntity player, ItemStack stack, NbtCompound nbt, LivingEntity npc, SoundEvent submitSound) {
        if (submitSound != null) {
            world.playSound(null, npc.getBlockPos(), submitSound, SoundCategory.NEUTRAL, 1.0f, 1.0f);
        }
