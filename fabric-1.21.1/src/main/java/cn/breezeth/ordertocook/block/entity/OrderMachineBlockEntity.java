...
public class OrderMachineBlockEntity extends BlockEntity implements NamedScreenHandlerFactory, ImplementedInventory {
    ...
    public boolean tryUpgrade(PlayerEntity player) {
        ...
        if (level >= MAX_LEVEL) {
            grantAdvancement(sp, Identifier.of(ModConstants.MOD_ID, "ordertocook/eight_star_restaurant"));
        }
    ...
    public boolean tryRename(ServerPlayerEntity player, String name) {
        ...
        if (!owner.isEmpty() && !owner.equals(pn)) {
            ...
            player.sendMessage(net.minecraft.text.Text.translatable("message.ordertocook.rename_not_owner").formatted(net.minecraft.util.Formatting.RED), true);
            return false;
        }
