...
public class RestaurantPersistentState extends SavedData {
    ...
    public static final SavedData.Factory<RestaurantPersistentState> TYPE =
            new SavedData.Factory<>(RestaurantPersistentState::new, RestaurantPersistentState::read, null);

    public static RestaurantPersistentState get(ServerLevel world) {
        var manager = world.getServer().overworld().getDataStorage();
        return manager.computeIfAbsent(TYPE, "ordertocook_restaurant_stats");
    }

    public static RestaurantPersistentState read(CompoundTag nbt, HolderLookup.Provider lookup) {
