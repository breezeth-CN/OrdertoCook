...
public final class ModClientNetworking {
    ...
    public static boolean sendPrestigeQuery() {
        ...
        PacketDistributor.sendToServer(ModNetworking.PrestigeQueryC2SPayload.INSTANCE);
        return true;
    }
    ...
    public static boolean sendRestaurantRankingQuery() {
        Minecraft client = Minecraft.getInstance();
        if (client.getConnection() == null) return false;
        PacketDistributor.sendToServer(ModNetworking.RestaurantRankingQueryC2SPayload.INSTANCE);
        return true;
    }
    ...
    public static boolean sendRestaurantRename(String name) {
        Minecraft client = Minecraft.getInstance();
        if (client.getConnection() == null) return false;
        PacketDistributor.sendToServer(new ModNetworking.RestaurantRenameC2SPayload(name));
        return true;
    }
    ...
    public static boolean sendRestaurantNameQuery() {
        Minecraft client = Minecraft.getInstance();
        if (client.getConnection() == null) return false;
        PacketDistributor.sendToServer(ModNetworking.RestaurantNameQueryC2SPayload.INSTANCE);
        return true;
    }
    ...
    public static void sendRiderSound(int soundType, float pitch) {
        ...
        PacketDistributor.sendToServer(new ModNetworking.RiderSoundPayload(soundType, pitch));
    }

    public static void sendRiderAnimation(int animType, boolean animate) {
        Minecraft client = Minecraft.getInstance();
        if (client.getConnection() == null) return;
        PacketDistributor.sendToServer(new ModNetworking.RiderAnimC2SPayload(animType, animate));
    }
